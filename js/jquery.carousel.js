(function ($) {
  $.fn.carousel = function (options) {
    /*
  ---horisontal - параметр, определяющий ориентацию слайдера (горизонтальную или вертикальную);
  по-умолчанию - горизонтальная;
  
  ---slideWidth - ширина слайда; используется как при горизонтальной так и при вертикальной ориентации слайдера;
  если указать в качестве значения 0 - будет использована фактическая ширина слайда; 
  
  ---imagesPerFrame - количество слайдов на экране; заданное значение используется только при вертикальном расположении слайдера;
  при горизонтальном расположении этот параметр рассчитывается как наибольшее количество слайдов на экране, либо, при малом количестве слайдов
  и не зацикленном показе как количество слайдов -1;
  
  ---shiftIncrement - сдвиг на каждом шаге прокрутки в единицах колличества слайдов; всегда меньше либо равен imagesPerFrame и больше 0;
  должен быть целым числом;
  
  ---cyclic - параметр, определяющий, будет ли прокрутка слайдов зациклена; по-умолчанию - будет;
  если прокрутка не зациклена то будет осуществляться следующим образом: есле скрытых слайдов еще достаточно - на величину shiftIncrement,
  иначе - так, чтобы стал виден последний слайд; затем, на следующем шаге лента возвращается в исходное крайнее положение;
  
  ---autoscrolling - параметр, определяющий, будет ли прокрутка слайдов выполнятся автоматически; по-умолчанию - нет;
  автоматическая прокрутка приостанавливается при наведении мыши на слайдер;
  
  ---autoscrollingSpeed - периодичность автоматической прокрутки в миллисекундах;
  
  ---slideDuration - длительность перемещения слайдов на каждом шаге;
  
  ---номер слайда и их общее количество отображается во всплывающей подсказке при наведении курсора мыши на соответствующий слайд;
  ...
  */
    ...
    function setCarousel(container) {
      ...
      options.imagesPerFrame = options.imagesPerFrame > 0 ? parseInt(options.imagesPerFrame) : 1;
      ...
      var shiftIncrement = options.shiftIncrement > 0 ? parseInt(options.shiftIncrement) : 1;
      shiftIncrement = shiftIncrement > imagesPerFrame ? imagesPerFrame : shiftIncrement;
      ...
      for (var i = 0; i < slidesCount; i++) {
        var slide = $('<div></div>').appendTo(ribbon).width(slideWidth).css({
          'float': 'left'
        });
      ...
      /*-------------уточнение параметров, установка необходимых обработчиков и запуск автоматической прокрутки*/
